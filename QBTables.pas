...
type
  ...
  public
    ...
    function AddFun(NameID: string; Color: TColor; Bold: Boolean; Size: Integer): Boolean;
   constructor Create;
   destructor Destroy; override;
  end;
  ...
  public
    ...
    function ProcessScript(Script: TStringList; ADDnoIDToTabList: Boolean;
    var NO_ID_COUNT,FOUND_ID_COUNT: Integer ): Boolean;
...
begin
    Result := False;
    if TabExists(ID) = False then begin
...
begin
  Result := 0;
  for B := 0 to High(Table) do begin
      HEXid := '';
      TabName := '';
      if Table[B] = 43 then begin    //2B - Inicio da ID
        for y := 1 to 4 do HEXId := HEXid + IntToHex(Table[B+y], 2);  //Copiar a HEX ID da tab
        for y := 5 to 50 do begin  //Copiar o Nome da tab, tamanho máximo 50
           if Table[B  + y] = 0 then Break;
           TabName := TabName + AnsiChar(Table[B + y]);
        end;
      end;
     if (HEXid <> '') and (TabName <> '') then begin
      if AddTable(HEXid,TabName) then Inc(Result, 1);
     end;
  end;
end;
...
begin
  ...
  try
    SetLength(Bytes, Stream.Size);
     ...
     for ID := 0 to High(Bytes) do begin
 ...
 for ID := 0 to High(Bytes) do begin   // varrer todos os bytes

    if (Bytes[ID] = 1) and (Bytes[ID+1] = 43) then begin   // Hex 01 2B - Inicio da table
        SetLength(BytesTables, High(Bytes) - ID +1 );
        for i := 0 to High(BytesTables) do begin   // Loop Inicio da ID até o fim do arquivo
...
begin
  ...
  if Tables.Count > 0 then begin
    ...
    for i := 0 to Tables.Count - 1 do begin
      ...
      if y > 0 then begin
        ID := Copy(Line, y +2, 8 );
        Name := Copy(Line, Pos('"', Line) +1 , Length(Line)-  Pos('"', Line) - 1) ;
        AddTable(ID, Name);
...
function TQBTables.ProcessScript(Script: TStringList; ADDnoIDToTabList: Boolean; var NO_ID_COUNT,FOUND_ID_COUNT: Integer ): Boolean;
var  i, y, x, z : Integer;
...
begin
  ...
  NO_ID_COUNT :=0;
  FOUND_ID_COUNT :=0;
  for i := 0 to Script.Count - 1 do  begin      //Varrer todas as linhas
      ...
      for x := 0 to Length(Line) - 1 do begin
         ...
         // if x+9 < High(Line) then
          if (Line[x] ='$') and (line[x + 9] = '$') then begin  //Pré verificação
            Temp := '';
            for z := x +1 to x + 8 do
            if Line[z] in ['0'..'9', 'A'..'F','a'..'f'] then Temp := Temp + Line[z]; //Copiar todos os digitos válidos
             if Length(Temp) = 8 then begin //Verificar se é uma TAB
               SetLength(ID_HEX, Length(ID_HEX) +1);
               ID_HEX[High(ID_HEX)] := Temp;         //Colocar na lista de IDS a serem buscadas
             end;
          end;
      end;
      //Buscar e substituir todas as IDS
      for y := 0 to Length(ID_HEX) - 1 do begin
         HEX_NAME := FindTable(AnsiUpperCase(ID_HEX[y]));
          if HEX_NAME = '' then begin
            HEX_NAME := 'NO_ID_FOUND_' + IntToStr(NO_ID_COUNT);
            Inc(NO_ID_COUNT, 1);
            if ADDnoIDToTabList then  AddTable(AnsiUpperCase(ID_HEX[y]), HEX_NAME);
            end else begin
            Inc(FOUND_ID_COUNT, 1);
          end;
         Line:= StringReplace(Line, ID_HEX[y], HEX_NAME,[]); //Substituir a ID HEX por Name
      end;
    Script[i] := Line;
