...
type
  ...
  private
    Tables: TQBTables;
    procedure AddAllFilesInDir(ListBox: TCheckListBox; FileExt: string; const Dir: string);
    procedure FormtarHTML(RichEdit: TJvRichEdit; xTextValue, xTag,
      DopCol: TColor);
...
begin
if ListBox2.Count < 1 then begin
...
ShowMessage('If a file decompile take more than 10 secs' + #13 + 'close the next window and unselect this file' + #13 + 'from the list, because probably this file is bugged.');
...
ScriptBat.Add('copy "' + ExtractFilePath(Application.ExeName) + edt_ToolName.Text + '" "' + MainFolder + '" /y ' );
   ...
   for I := 0 to ListBox2.Count - 1 do  begin
     ...
     FileName := TPath.GetFileNameWithoutExtension(ListBox2.Items[i]);
      ScriptBat.Add('echo. ');
      ScriptBat.Add('echo    ' + IntToStr(i)+ ' of ' + IntToStr(ListBox2.Count) + ' files processed');
      ScriptBat.Add('echo    Decompiling file '+ FileSubDir + FileName + '.qb ');
      ScriptBat.Add(edt_ToolName.Text + ' ' + edt_QbArgument.Text + ' ' + FileSubDir + FileName + '.qb > ' + FileSubDir + FileName + '.txt' );
...
if (Components[i] is TJvFilenameEdit) then
mmo1.Lines.Add(Components[i].Name + ',' + (Components[i] as TJvFilenameEdit).Text + ',' + Components[i].ToString);
if (Components[i] is TJvDirectoryEdit) then
mmo1.Lines.Add(Components[i].Name + ',' + (Components[i] as TJvDirectoryEdit).Text + ',' + Components[i].ToString);
if (Components[i] is TEdit) then
mmo1.Lines.Add(Components[i].Name + ',' + (Components[i] as TEdit).Text + ',' + Components[i].ToString);
if (Components[i] is TCheckBox) then
mmo1.Lines.Add(Components[i].Name + ',' + BoolToStr((Components[i] as TCheckBox).Checked, True) + ',' + Components[i].ToString);
end;
end;
end;
...
begin
if High(Tables.Table) < 0 then  begin
  ...
  pb1.Position := 50;
  lblFile.Caption := (IntToStr(High(Tables.Table)) + ' tables loaded. Loading script file...');
  Application.ProcessMessages;
 Tables.ProcessScript(Script, cb_addUnknownTables.Checked, NO_ID_COUNT, FOUND_ID_COUNT);

  pb1.Position := 0;
  Application.ProcessMessages;
  rcScriptConverted.Lines.Text := Script.Text;
  lblFile.Caption := 'Processed tables: ' + IntToStr(FOUND_ID_COUNT + NO_ID_COUNT)
    + '           Found: ' + IntToStr(FOUND_ID_COUNT)
    + '           Not found: ' + IntToStr(NO_ID_COUNT);
  Application.ProcessMessages;
  ts5.TabVisible := True;
  pgc2.ActivePage := ts5;
  Script.Free;
...
begin
  ...
  begin
    ...
    FoundAt :=
      rce_ViewFile.FindText(FindDialog1.FindText, StartPos, ToEnd, mySearchTypes);
    if FoundAt <> -1 then
    begin
      SetFocus;
      SelStart := FoundAt;
...
MainTreeNode := tvTextMach.Items.Add(nil, 'Matching results for "' +edtTextToFind.Text + '"');

if CaseSensitive then
TextToSeek := edtTextToFind.Text else TextToSeek := AnsiUpperCase(edtTextToFind.Text);
...
pb1.Position := 0;
 for i := 0 to ListBox.Count - 1 do begin
   ...
   for y := 0 to TextStrings.Count - 1 do begin
      if CaseSensitive then FindPos := Pos(TextToSeek, TextStrings[Y])
      else FindPos := Pos(TextToSeek, AnsiUpperCase(TextStrings[Y]));
      if FindPos > 0 then begin
       if LastFileFound = ListBox.Items[I] then begin
             tvTextMach.Items.AddChild(SubTreeNode, ' Line: ' + IntToStr(Y +1) + ' Pos: ' + IntToStr(FindPos) );
       end else begin
       SubTreeNode:= tvTextMach.Items.AddChild(MainTreeNode,ListBox.Items[i]);
       tvTextMach.Items.AddChild(SubTreeNode, ' Line: ' + IntToStr(Y +1) + ' Pos: ' + IntToStr(FindPos) );
 ...
 MainTreeNode.Text := 'Matching results for "' +edtTextToFind.Text + '" (' + IntToStr(ItensFound) + ' results found)';
ListBox.Free;
TextStrings.Free;
...
begin
Allowed := False;
if Key in ['0'..'9', #8, #127] then Allowed := True;
if Allowed = False then Abort;
...
begin
if Key = #13 then FindText(Sender);
...
begin
 ...
 if Tables.LoadFromFile(edt2.FileName) then begin
...
if cb_autoReplace.Checked then   btnConvertScript(Sender);
...
Var Selected, ParentSelected, Arg, Line, Column: string;
TextLine, TextPos: Integer;
begin

if Assigned(tvTextMach.Selected) then begin
 if chk_customEditor.Checked = false then begin
  ...
  if AnsiUpperCase(ExtractFileExt(ParentSelected)) = '.TXT' then begin
    ...
    PG_FindString.ActivePage := TS_ViewFile;
     ...
     if  AnsiUpperCase(ExtractFileExt(Selected)) = '.TXT' then begin
  ...
  try
     if ( Copy(Selected, 8, Pos('Pos:', Selected) -9) ) <> '' then  begin

     TextLine := StrToInt( Copy(Selected, 8, Pos('Pos:', Selected) -9) );
     rce_ViewFile.SelStart := rce_ViewFile.Perform(EM_LINEINDEX, TextLine, 0);
     end;
   finally
      //Silent Discart
   end;
  ...
  Column :=  Copy(Selected, Pos('Pos:', Selected) + 5, Length(Selected) - Pos('Pos:', Selected));
  Arg := StringReplace(edt_CVArg.Text, '%FILEPATH%', '"' + ParentSelected + '"', [rfReplaceAll, rfIgnoreCase]);
  Arg := StringReplace(Arg, '%LINE%', Line, [rfReplaceAll, rfIgnoreCase]);
  Arg := StringReplace(Arg, '%COLUMN%',Column  , [rfReplaceAll, rfIgnoreCase]);
   ShellExecute(handle,'open',PChar(edt_editorPath.Text), PChar(Arg),'',SW_NORMAL);
  end else begin
  ShellExecute(handle,'open',PChar(edt_editorPath.Text), PChar(ParentSelected),'',SW_NORMAL);
 end;
...
procedure TForm3.tvTextMachKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (Key = 70) and (Shift = [ssCtrl]) then begin
  FindDialog1.Position := Point(rce_ViewFile.Left + rce_ViewFile.Width, rce_ViewFile.Top);
  FindDialog1.Execute;
 end;
end;
...
procedure TForm3.AddAllFilesInDir(ListBox: TCheckListBox; FileExt: string; const Dir: string);
var
  SR: TSearchRec;
begin
    ...
    try
      repeat
        ...
        if ((SR.Attr and faDirectory) = 0) then  begin
         if ExtractFileExt(SR.Name)  = FileExt then ListBox.Items.Add(Dir + '\' + SR.Name)
        end else if (SR.Name <> '.') and (SR.Name <> '..') then
          AddAllFilesInDir(ListBox, FileExt, IncludeTrailingBackslash(Dir) + SR.Name);  // recursive call!
      until FindNext(Sr) <> 0;
    finally
      FindClose(SR);
    end;
...
procedure TForm3.FormtarHTML(RichEdit: TJvRichEdit; xTextValue, xTag,DopCol: TColor);
